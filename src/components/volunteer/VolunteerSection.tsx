...
export function VolunteerSection({ campaigns, onToast }: VolunteerSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (type === 'support') {
        ...
        onToast('پیام شما دریافت شد. به زودی پاسخ داده خواهد شد.', 'success');
      } else {
        await api.submitVolunteer({
  ...
  const TABS: { key: FormType; label: string; icon: any }[] = [
    { key: 'physical', label: 'داوطلب حضوری', icon: Users },
    { key: 'inkind', label: 'اهدای کالا', icon: Package },
    { key: 'support', label: 'پیام به پشتیبانی', icon: MessageCircle },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 bg-white/5 rounded-3xl border border-white/10">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-[#8B7355]/20 rounded-2xl flex items-center justify-center">
                  <MessageCircle className="w-5 h-5 text-[#D2B48C]" />
                </div>
                <h3 className="font-bold text-lg">پیام به پشتیبانی</h3>
              </div>
              <p className="text-stone-400 text-sm leading-relaxed">
                سوال، پیشنهاد یا هر موضوعی دارید؟ پیام بدید تا در اسرع وقت پاسخ دهیم.
              </p>
            </div>
          </div>
            ...
            {done ? (
              <div className="py-8 text-center space-y-3">
                ...
                <p className="font-bold text-lg">
                  {type === 'inkind' ? 'درخواست اهدای کالا ثبت شد'
                    : type === 'support' ? 'پیام شما دریافت شد'
                    : 'داوطلبی شما ثبت شد'}
                </p>
                <p className="text-stone-400 text-sm">
                  {type === 'inkind' ? 'برای هماهنگی تحویل اقلام با شما تماس گرفته خواهد شد.'
                    : type === 'support' ? 'به زودی پاسخ داده خواهد شد.'
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Campaign selector — for physical and inkind */}
                {type !== 'support' && (
                  <div>
                ...
                <div>
                  <label className="block text-xs font-bold text-stone-400 mb-1.5">
                    {type === 'physical' ? 'توانایی‌ها و زمان‌بندی شما'
                      : type === 'inkind' ? 'شرح کالا یا اقلام'
                      : 'پیام شما'}
                  </label>
                  <textarea required name="message" rows={3}
                    ...
                    placeholder={
                      type === 'physical' ? 'مثلاً: آخر هفته‌ها آزادم، می‌توانم در آواربرداری یا توزیع بسته کمک کنم.'
                      : type === 'inkind' ? 'مثلاً: ۱۰۰ کیلو برنج آماده اهدا دارم.'
                ...
                <button type="submit" disabled={loading}
                  ...
                  {type === 'physical' ? 'ثبت داوطلبی'
                    : type === 'inkind' ? 'ثبت درخواست اهدا'
                    : 'ارسال پیام'}
                </button>
              </form>
            )}
